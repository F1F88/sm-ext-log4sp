...
static cell_t BaseFileSink_GetFilename(IPluginContext *ctx, const cell_t *params)
{
    ...
    {
        auto sink = std::dynamic_pointer_cast<spdlog::sinks::basic_file_sink_st>(sinkAdapterPtr->raw());
        if (sink == nullptr)
        {
    ...
    {
        auto sink = std::dynamic_pointer_cast<spdlog::sinks::basic_file_sink_mt>(sinkAdapterPtr->raw());
        if (sink == nullptr)
        {
